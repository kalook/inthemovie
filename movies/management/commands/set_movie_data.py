...
class Command(BaseCommand):
	...
	def handle(self, *args, **options):
		...
		#수집 초기화
		now_show = Movies.objects.filter(is_collected = 'T', show = 'N')
		for now in now_show:
			now.is_collected = 'F'
			now.save()
		
		#영화명 수집
		pre_movies = self.daum.get_daum_movie('pre_movie')
		curr_movies = self.daum.get_daum_movie('curr_movie')
		
		self._config_movie_name(pre_movies,'naver_premovie')
		print "--------------------------------------------------"
		self._config_movie_name(curr_movies,'naver_current')
		print "///////////////////////////////////////////////////"
		movie_list = Movies.objects.filter(is_collected = 'T')
		
		for movie in movie_list:
			title_eng = link_naver = thumnail = link_imdb = link_movist =''
				...
				for blog in blog_api:	
					try:
						...
						data = CollectData.objects.create(
							...
							subject = (blog_api[blog]).get('title'),content_url = '',content_date = (blog_api[blog]).get('pubDate'))
						data.save()
	...
	def _config_movie_name(self, movies, types):
		for movie in movies:
			...
			try:
				print u'이름수집중'.encode("utf-8") ,title
			except:
				pass
			...
			try:
				...
				for list in movie_title:
					list.is_collected = 'T'
					list.type = types
					list.link_daum = movie.get('href')
					list.save()
					try:
						print list.title_kor + ' ==> collected'		
					except:
						pass
				if not movie_title:
					try:
		...
		if title_eng:
			...
			for kor_letter in title_kor:
				try:
					...
					print eng_lett.eng_letter
					kor_letter_title = kor_letter_title + str(eng_lett.eng_letter)
